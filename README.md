...
Blog posts are stored in the MySQL `BlogPost` table. They are not saved as files in the project or in Hostinger's `uploads` directory. The Media Library stores file metadata in the `MediaAsset` table and writes uploaded files to `HOSTINGER_IMAGE_UPLOAD_DIR` when the app is running on Hostinger. During local development, uploads fall back to `public/uploads/images`.
