...
function Editor({ form, change, authors, save, upload, removeUploadedImage, busy, message, cancel }: { form: Form; change: (key: keyof Form, value: string | null) => void; authors: Author[]; save: (event: React.FormEvent) => void; upload: (file: File) => Promise<string | null>; removeUploadedImage: (url: string) => Promise<void>; busy: boolean; message: string; cancel: () => void }) {
  ...
  const editor = useEditor({ extensions: [StarterKit.configure({ link: false }), Image, Link.configure({ openOnClick: false }), Table.configure({ resizable: true }), TableRow, TableHeader, TableCell, Youtube.configure({ controls: true, nocookie: true })], content: documentMode ? '<p>Use HTML source mode to edit this document.</p>' : normalizeBlogHtml(form.body || '<p></p>'), onUpdate: ({ editor: current }) => { if (!documentMode) { change('body', current.getHTML()); setDirty(true); setSelectionVersion((version) => version + 1); } }, onSelectionUpdate: () => setSelectionVersion((version) => version + 1), editorProps: { attributes: { class: 'prose min-h-[300px] max-w-none p-4 outline-none' } } });
  ...
  const toggleSource = () => { if (!documentMode && source && editor) editor.commands.setContent(normalizeBlogHtml(form.body || '<p></p>'), { emitUpdate: false }); setSource((value) => !value); };
  const inlineUpload = async (file: File) => { const url = await upload(file); if (url) { if (source) { change('body', `${form.body}\n<img src="${url}" alt="">`); setDirty(true); } else editor?.chain().focus().setImage({ src: url }).run(); } };
  ...
  return <form onSubmit={save}>{message && <p className="mb-5 rounded-[4px] border border-[#e7b5a6] bg-[#fff3ef] p-3 text-[13px] text-[#9f3d3d]">{message}</p>}{bodySizeWarning && <p className="mb-5 border border-[#ead7ad] bg-[#fffaf0] p-3 text-[13px] text-[#8a5a00]">{bodySizeWarning}</p>}
