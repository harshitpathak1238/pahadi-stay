...
const empty = (): Form => ({ slug: '', title: '', metaTitle: '', metaDescription: '', excerpt: '', body: '<p></p>', authorName: 'Kainchi Darshan Team', authorId: null, category: 'Blog', primaryKeyword: '', tags: '', featuredImage: null, imageAltText: '', status: 'PUBLISHED', scheduledAt: null, publishedAt: null });
...
function Editor({ form, change, authors, save, upload, removeUploadedImage, busy, message, cancel }: { form: Form; change: (key: keyof Form, value: string | null) => void; authors: Author[]; save: (event: React.FormEvent) => void; upload: (file: File) => Promise<string | null>; removeUploadedImage: (url: string) => Promise<void>; busy: boolean; message: string; cancel: () => void }) {
  ...
  const [fullDocumentParts, setFullDocumentParts] = useState<FullBlogDocumentParts | null>(() => documentMode ? splitFullBlogDocument(form.body) : null);
  ...
  const editor = useEditor({ extensions: [StarterKit.configure({ link: false }), GenericArticle, GenericDiv, GenericSpan, ResizableImage, Link.configure({ openOnClick: false }), Table.configure({ resizable: true }), TableRow, TableHeader, TableCell, Youtube.configure({ controls: true, nocookie: true })], content: documentMode ? fullDocumentParts?.body || '<p></p>' : normalizeBlogHtml(form.body || '<p></p>'), onUpdate: ({ editor: current }) => { if (!source) { const body = current.getHTML(); if (documentMode) setFullBodyChanged(true); change('body', documentMode && fullDocumentParts ? `${fullDocumentParts.prefix}${body}${fullDocumentParts.suffix}` : body); setDirty(true); setSelectionVersion((version) => version + 1); } }, onSelectionUpdate: () => setSelectionVersion((version) => version + 1), editorProps: { attributes: { class: 'prose min-h-[300px] max-w-none p-4 outline-none' } } });
  useEffect(() => { if (editor && !source && documentMode && !fullBodyChanged && fullDocumentParts && editor.getHTML() !== fullDocumentParts.body) editor.commands.setContent(fullDocumentParts.body, { emitUpdate: false }); }, [documentMode, editor, fullDocumentParts, fullBodyChanged, source]);
  useEffect(() => { if (editor && !source && !documentMode) { const content = normalizeHtml(form.body || '<p></p>'); if (editor.getHTML() !== content) editor.commands.setContent(content, { emitUpdate: false }); if (content !== form.body) change('body', content); } }, [documentMode, editor, form.body, source]);
  const active = (name: string) => { void selectionVersion; return Boolean(editor?.isActive(name)); };
  const toggleSource = () => {
    if (source) {
      if (isFullBlogDocument(form.body)) {
        const parts = splitFullBlogDocument(form.body);
        setFullDocumentParts(parts);
        setFullBodyChanged(false);
        editor?.commands.setContent(parts.body, { emitUpdate: false });
        setSourceNotice(false);
      } else if (editor) {
        const normalized = normalizeBlogHtml(form.body || '<p></p>');
        editor.commands.setContent(normalized, { emitUpdate: false });
        change('body', normalized);
        setSourceNotice(normalized !== form.body);
      }
    } else if (documentMode && editor && fullDocumentParts) {
      const body = fullBodyChanged ? editor.getHTML() : fullDocumentParts.body;
      change('body', `${fullDocumentParts.prefix}${body}${fullDocumentParts.suffix}`);
    } else if (editor) {
      change('body', editor.getHTML());
    }
  ...
  const editorStyles = (fullDocumentParts?.styles || '').replace(/\b(?:html|body)\b/gi, '.full-blog-editor');
  const field = (key: keyof Form, label: string, _required = false, minLength?: number, maxLength?: number) => { const optionalMeta = ['metaTitle', 'metaDescription', 'primaryKeyword'].includes(String(key)); return <label className="grid gap-1 text-[12px] font-semibold">{label}<input required={false} minLength={optionalMeta ? undefined : minLength} maxLength={maxLength} value={String(form[key] ?? '')} onChange={(event) => { const value = event.target.value; change(key, value); if (key === 'title') { if (!form.id) change('slug', slugify(value)); change('metaTitle', value); } setDirty(true); }} className="h-9 border border-[#d9d9dc] bg-white px-2 text-[13px] font-normal" /></label>; };
