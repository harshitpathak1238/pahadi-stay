...
export function MediaLibrary() {
  ...
  const allPageSelected = assets.length > 0 && assets.every((asset) => selectedIds.includes(asset.id));
  const toggleAllPage = () => setSelectedIds((current) => allPageSelected ? current.filter((id) => !assets.some((asset) => asset.id === id)) : Array.from(new Set([...current, ...assets.map((asset) => asset.id)])));
  ...
  return <section className="relative min-h-screen bg-[#f6f6f7]">
    ...
    {!assets.length ? <div className="border border-[#e1e1e3] bg-white py-16 text-center"><FileImage className="mx-auto text-[#999]" size={30} /><p className="mt-3 text-[14px] font-semibold">No files yet - upload your first image or video</p></div> : view === 'grid' ? <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">{assets.map((asset) => <div key={asset.id} className={`relative overflow-hidden border bg-white text-left ${selectedIds.includes(asset.id) ? 'border-[#24584a] ring-2 ring-[#b9d5c5]' : 'border-[#e1e1e3]'}`}><label className="absolute left-2 top-2 z-10 grid h-6 w-6 cursor-pointer place-items-center bg-white/95 shadow-sm"><input type="checkbox" checked={selectedIds.includes(asset.id)} onChange={() => toggleSelected(asset.id)} aria-label={`Select ${asset.filename}`} /></label><button type="button" onClick={() => setSelected(asset)} className="group block w-full text-left"><div className="relative aspect-square bg-[#f0f0f1]">{asset.mimeType.startsWith('video/') ? <video src={asset.url} className="h-full w-full object-cover" muted preload="metadata" /> : <img src={asset.thumbnailUrl || asset.url} alt={asset.altText || ''} className="h-full w-full object-cover" />}<span className="absolute bottom-2 left-2 bg-[#303030]/85 px-2 py-1 text-[10px] text-white">{asset.usage.length ? `Used ${asset.usage.length} time${asset.usage.length === 1 ? '' : 's'}` : 'Unused'}</span></div><div className="p-3"><p className="truncate text-[12px] font-semibold">{asset.filename}</p><p className="mt-1 text-[11px] text-[#777]">{formatSize(asset.size)} - {new Date(asset.createdAt).toLocaleDateString('en-IN')}</p></div></button></div>)}</div> : <div className="mt-4 overflow-x-auto border border-[#e1e1e3] bg-white"><table className="w-full text-left text-[12px]"><thead className="border-b border-[#e1e1e3] bg-[#fafafa] text-[10px] uppercase text-[#777]"><tr><th className="p-3"><input type="checkbox" checked={allPageSelected} onChange={toggleAllPage} aria-label="Select all files on this page" /></th><th className="p-3">File</th><th className="p-3">Type</th><th className="p-3">Size</th><th className="p-3">Dimensions</th><th className="p-3">Uploaded</th><th className="p-3">Usage</th></tr></thead><tbody>{assets.map((asset) => <tr key={asset.id} onClick={() => setSelected(asset)} className="cursor-pointer border-b border-[#eee]"><td className="p-3" onClick={(event) => event.stopPropagation()}><input type="checkbox" checked={selectedIds.includes(asset.id)} onChange={() => toggleSelected(asset.id)} aria-label={`Select ${asset.filename}`} /></td><td className="p-3 font-semibold">{asset.filename}</td><td className="p-3">{asset.mimeType}</td><td className="p-3">{formatSize(asset.size)}</td><td className="p-3">{asset.width && asset.height ? `${asset.width} x ${asset.height}` : '-'}</td><td className="p-3">{new Date(asset.createdAt).toLocaleDateString('en-IN')}</td><td className="p-3">{asset.usage.length}</td></tr>)}</tbody></table></div>}
