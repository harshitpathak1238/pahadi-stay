...
function MediaDetail({ asset, onClose, onSaved, onDelete }: { asset: Asset; onClose: () => void; onSaved: () => void; onDelete: () => void }) {
  ...
  const save = async () => { setSaving(true); setSaveError(''); const data = new FormData(); data.append('id', asset.id); data.append('filename', filename.trim()); data.append('altText', altText); if (replacement) data.append('file', replacement); try { const response = await fetch('/api/admin/media', { method: 'POST', body: data }); const result = await response.json().catch(() => ({})); if (!response.ok) { setSaveError(result.error || `Could not save changes (${response.status}).`); return; } onSaved(); } catch { setSaveError('Could not reach the media service.'); } finally { setSaving(false); } };
