...
import { ArrowLeft, ArrowDown, ArrowUp, Bath, BedDouble, BedSingle, Bold, CircleParking, Code2, ConciergeBell, Flower2, GripVertical, ImagePlus, Info, Italic, Languages, Link as LinkIcon, List, ListOrdered, Monitor, Plus, Quote, Save, Search, Star, Trash2, Upload, UserRound, Wifi, type LucideIcon } from 'lucide-react';
...
export function CategoryListingEditor({ category, form, setForm, busy, message, cancel, save, editingId }: Props) {
  ...
  useEffect(() => {
    ...
    }).catch(() => { /* count stays null — button still opens the modal */ });
    return () => { active = false; };
  ...
  return (
    <form onSubmit={save} className="rounded-2xl border border-[#dfe3d8] bg-white p-5 md:p-7 [&_b]:hidden">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        {category === 'STAY' && <div className="md:col-span-2 rounded-2xl border border-[#dfe3d8] bg-[#f7f8f4] p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-[13px] font-bold text-[#173f35]">Guest reviews <span className="font-normal text-[#6c7770]">(moderation)</span></p><p className="mt-1 text-[11px] font-normal text-[#6c7770]">Approve, reject, edit, or add reviews for this stay. Each action saves immediately — it does not touch this form's Save button. Public stay page updates without a rebuild.</p></div><button type="button" disabled={!editingId} onClick={() => setReviewsOpen(true)} className="inline-flex items-center gap-2 rounded-xl bg-[#173f35] px-4 py-2 text-xs font-bold text-white hover:bg-[#0d241d] disabled:opacity-40"><Star size={14} /> Manage reviews{reviewCount !== null ? ` (${reviewCount})` : ''}</button></div>{!editingId && <p className="mt-3 rounded-xl border border-dashed border-[#c9c9cc] bg-white p-3 text-center text-xs text-[#777]">Save this stay once to unlock review management.</p>}</div>}
