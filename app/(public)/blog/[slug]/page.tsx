...
export default async function Article({ params }: { params: { slug: string } }) {
  ...
  return <article className="mx-auto max-w-3xl px-5 py-14 md:py-20"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /><p className="sans text-xs font-bold uppercase tracking-[.2em] text-[#b66b45]">{blog.category} · By {blog.authorName}</p><h1 className="mt-4 text-5xl leading-[1.02] text-[#173f35] md:text-6xl">{blog.title}</h1><p className="mt-8 text-xl leading-8 text-[#526057]">{blog.excerpt}</p>{blog.featuredImage && <div className="relative mt-10 aspect-[16/9] overflow-hidden rounded-2xl"><Image src={blog.featuredImage} alt={blog.imageAltText || blog.title} fill priority sizes="(max-width: 768px) 92vw, 768px" className="object-cover" /></div>}<div className="mt-12 space-y-7 sans text-base leading-8 text-[#526057]">{paragraphs.map((paragraph, index) => <p key={`${blog.slug}-${index}`}>{paragraph}</p>)}</div></article>;
}
