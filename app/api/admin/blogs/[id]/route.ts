...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const existing = await db.blogPost.findUnique({ where: { id: params.id }, select: { slug: true, publishedAt: true } });
    if (!existing) return NextResponse.json({ error: 'Blog not found.' }, { status: 404 });
    const data = { ...parsed.data, publishedAt: parsed.data.status === 'PUBLISHED' ? existing.publishedAt ?? new Date() : null, scheduledAt: parsed.data.status === 'SCHEDULED' ? parsed.data.scheduledAt : null };
