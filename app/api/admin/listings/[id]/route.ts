...
const updateSchema = z.object({ slug: z.string().trim().max(160).optional(), category: z.preprocess((value) => typeof value === 'string' ? value.trim().toUpperCase() : value, z.enum(['STAY', 'RIDE', 'RENTAL', 'ACTIVITY']).optional()), title: z.string().trim().min(1, 'Title is required.').max(120).optional(), description: z.string().optional(), location: z.string().trim().min(1, 'Location is required.').max(160).optional(), address: z.string().trim().min(1, 'Address is required.').max(500).optional(), basePrice: z.coerce.number().nonnegative().optional(), sellPrice: z.coerce.number().nonnegative().optional(), images: optionalStringList, amenities: optionalStringList, details: z.record(z.string(), z.unknown()).optional(), seoTitle: z.string().trim().max(160).optional(), seoDescription: z.string().trim().max(320).optional(), cancellationPolicy: z.string().trim().optional(), featured: z.coerce.boolean().optional(), partnerId: z.string().trim().min(1).optional(), status: updateStatus }).superRefine((value, context) => { if (value.basePrice !== undefined && value.sellPrice !== undefined && value.sellPrice < value.basePrice) context.addIssue({ code: z.ZodIssueCode.custom, path: ['sellPrice'], message: 'Selling price must be greater than or equal to base price.' }); });
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const existing = await db.listing.findUnique({ where: { id: params.id }, select: { category: true, title: true, description: true, basePrice: true, sellPrice: true, images: true, location: true, details: true } });
  if (!existing) return NextResponse.json({ error: 'Listing not found.' }, { status: 404 });
  const candidate = { ...existing, ...parsed.data, category: parsed.data.category || existing.category, details: parsed.data.details || existing.details, images: parsed.data.images || existing.images };
  const missing = parsed.data.status === 'LIVE' ? listingLiveRequirements(candidate.category, candidate) : [];
