...
export async function GET() {
  ...
  const [bookings, revenue, previousRevenue, pendingPickups, pendingPartners, failedPayments, recentOrders, categoryGroups, dailyRevenue] = await Promise.race([
    Promise.all([
    ...
    new Promise<never>((_, reject) => setTimeout(() => reject(new Error('Database request timed out.')), 8000)),
  ]);
